...
import { Shield, User as UserIcon, Mail, Phone, Clock, Briefcase, Award, BadgeCheck } from 'lucide-react';
...
export default function ManagerProfilePage() {
  ...
  const initial = currentUser.name?.[0] || currentUser.email?.[0] || 'M';
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-slate-800 bg-slate-900/90 p-4 sm:p-5 shadow-lg">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-white">My Operations Manager Profile</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            View your registered manager credentials and account privileges.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-md flex flex-col items-center justify-center text-center">
          ...
          <div className="w-full border-t border-slate-850 mt-5 pt-4 text-xs text-slate-400 space-y-2.5 text-left">
            ...
            <div className="flex justify-between">
              <span>Title:</span>
              <span className="font-semibold text-slate-200">{currentUser.title || 'Operations Manager'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-md space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-850">
              ...
              <span className="text-white font-medium">{currentUser.department || 'Operations Management'}</span>
            </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-850">
              ...
              <span className="text-white font-medium">{currentUser.title || 'Operations Manager'}</span>
            </div>
